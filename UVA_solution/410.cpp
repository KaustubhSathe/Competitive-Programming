...
int main(){
    ...
    while(cin>>c>>s){
        ...
        for(int j = 0;j<numbers.size()/2;j++){
            if(numbers[j] == 0 && numbers[numbers.size()-j-1] == 0){
                printf("%d: \n",j);
                imbalance += abs(numbers[j] + numbers[numbers.size()-j-1] - (double)sum/c);
            }else if(numbers[j] == 0){
                printf("%d: %d\n",j,numbers[numbers.size()-j-1]);
                imbalance += abs(numbers[j] + numbers[numbers.size()-j-1] - (double)sum/c);
            }else{
                printf("%d: %d %d\n",j,numbers[j],numbers[numbers.size()-j-1]);
                imbalance += abs(numbers[j] + numbers[numbers.size()-j-1] - (double)sum/c);
            }
        }
